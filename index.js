...
let typoIndex = artists.findIndex((artist) => artist.name === "Vincent van Dough");
artists[typoIndex].name = "Vincent van Gogh";
console.log(artists[typoIndex]);
...
function get20s(array){
  ...
  {
    //console.log(array[i].years.split(" ")); // "years" is stored in the format "BORN - DIED"
    if(Number(array[i].years.split(" ")[0]) >= 1900) // extract "BORN" using the split, then cast it to a number and compare it to 1900
    {                                                // by saving array[i].years.split and then using .length we could get the last element of the split (the death year)
      result.push(array[i].name);                    // but since there are no artists d. > 2000 in this data, it's unnecessary.
    }
  }
  return result;
}
...
function addArtist(artists, id, name, years, genre, nationality, bio){
  const newArtist = {id, name, years, genre, nationality, bio};
  artists.push(newArtist);
  return artists;
}

console.log(addArtist(artists, 20, "Sam Kester", "1991 - 2020", "Web Design", "United States", "This is a test bio; it has nothing to do with reality. In this it is much like other artists' autobiographies."));
...
function randomize(array){
  ...
  while(array.length > 0)
  {
    ...
    //                         generate a random index in the starting array...
    //            ...remove the element from that position...
    //...and push it to the end of the new array.
    //                                                                     [0] goes from the single-item array given by splice(,1) to the item it contains
  }
  return newArray;
}
